...
export class LeadService implements ILeadService{
    ...
    async getLeadByStatus(status: number = 7, page: number = 1, limit: number = 10): Promise<any> {
        try {
            const response = await this.__leadRepository.getLeadByStatus(status,page,limit)
