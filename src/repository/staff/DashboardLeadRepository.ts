...
import { IDashboardLeadRepository } from "../interface/staff/IDashboardLeadRepository";


class DashboardLeadRepository implements IDashboardLeadRepository{

    async getDashboardLeadData(staffId: string) {
  try {
    const leadStats = await LeadModel.aggregate([
      {$match: {status: {$in :[1,2,3,4,5,6]}, assignedAgent : new mongoose.Types.ObjectId(staffId)}},
      {
        $group: {
          _id: null,
          totalLeads: { $sum: 1 },
    ...
    const data = await LeadModel.aggregate([
      {
        $match: {
          ...
          assignedAgent : new mongoose.Types.ObjectId(staffId)
        }
      },
      {
        $group: {
